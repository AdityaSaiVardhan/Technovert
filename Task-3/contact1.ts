...
function genderFun(input:HTMLInputElement){
	if(input.id=='male'){
		alert('Hello sir!');
	}
...
function showemptymsg(error_id:string,string:string)
{
...
function showerror(error_id:string,string:string){
   (<HTMLDivElement>document.getElementById(error_id)).style.display="inline";
   (<HTMLDivElement>document.getElementById(error_id)).innerHTML="Invalid".concat(" ",string);  
}

function hideerror(error_id:string){
