$('#contact_state').on("change",function(input:JQuery.ChangeEvent):void{
...
$('#male,#female').on("click",function gender(input:JQuery.ClickEvent):void{
...
var org_error:boolean=false;
var webAddr_error:boolean=false;

$('#contact_name_field').on("keyup",function(input:JQuery.KeyUpEvent):void
{
...
$('#contactform').on("keyup",function(input:JQuery.KeyUpEvent):void
{
   var email_reg:RegExp=/^([a-zA-Z0-9_\-\.])+\@([a-zA-Z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
   var org_reg:RegExp=/^[a-zA-Z\s]*$/;
   var webAddr_reg:RegExp=/^(?:http(s)?:\/\/)?[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\(\)\*\+,;=.]+$/;
...
$('#contact_phone_number_field').on("keyup",function(input:JQuery.KeyUpEvent):void
{
...
$('#submit').on("click",function(event:JQuery.ClickEvent):void{
...
$('#reset').on("click",function(event:JQuery.ClickEvent):void {  
   var response:boolean=confirm("Are you sure?");
	var inputFields:any =$(".input_textbox").val();
	if(response){
